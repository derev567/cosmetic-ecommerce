...
export const Reviews = ({productId,averageRating}) => {
    ...
    console.log("the user reviews are somthgin",reviews)
    console.log("the user reviews are somthgin",reviews[0])
    console.log("the user reviews are somthgin",reviews[1])
        ...
        <Stack rowGap={5} alignSelf={"flex-start"}  width={is480?"90vw":is840?"25rem":'40rem'}>
            ...
            {reviews?.map(
                ...
                if(review)
                {
                    if(review.user)
                    {  
                        return (<ReviewItem key={review._id} id={review._id} userid={review.user._id} comment={review.comment} createdAt={review.createdAt} rating={review.rating} username={review.user.name} />)
                    }
